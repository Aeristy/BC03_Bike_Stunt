...
namespace Kamgam.BikeAndCharacter25D
{
    public partial class Bike : MonoBehaviour
    {
        ...
        protected void loadConfig()
        {
            ...
            configBoostIncrement = Config.BikeMaxVelocityWithBoostMultiplier;
        ...
        {
            float speedMultiply = IsBoost ? configBoostIncrement : 1.0f;
            ...
            bool isUnderSpeedLimit = lookingDirectionIsOppositeToVelocity ||
                Velocity < configMaxVelocity * speedMultiply;
            if (IsTouchingGround && isSpeedingUp && isUnderSpeedLimit)
            {
                nextMotorSpeedIncrement = configMotorSpeedIncrement * speedMultiply;
            }
            else
            {
            ...
            if (Velocity < configMaxVelocity * speedMultiply && BackWheelBody.angularVelocity < -motorSpeed)
            {
                motorSpeed = -BackWheelBody.angularVelocity;
            }
            motorSpeed = Mathf.Clamp(motorSpeed + nextMotorSpeedIncrement * speedMultiply, 0, configMaxMotorSpeed * speedMultiply);
            ...
            if (nextMotorSpeedIncrement > 0)
            {
                    ...
                    )
                {
                    ...
                    m.maxMotorTorque = configMaxMotorTorque * speedMultiply;
                        ...
                        && Velocity < 40 * speedMultiply)
                    {
                        ...
                        if (SlopeInDegrees > 30 && SlopeInDegrees < 90)
                        {
                            ...
                            m.maxMotorTorque = configMaxMotorTorqueOnSlopes * speedMultiply;
                            BackWheelJoint.motor = m; // automatically sets useMotor to true
                        }
                    }
                }
