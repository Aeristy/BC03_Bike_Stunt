				...
				while (currentX < maxX)
				{
                    ...
                    foreach (var hit in hits)
                    {
						...
						if (1 - Vector3.Project(normal.normalized, Vector3.up).magnitude < Settings.SlopeLimit)
						{
							...
							obj.transform.Rotate(0, Random.Range(-0.5f * Settings.RotYVarianceInDeg, 0.5f * Settings.RotYVarianceInDeg), Random.Range(-0.5f * Settings.RotZVarianceInDeg, 0.5f * Settings.RotZVarianceInDeg), Space.Self);
